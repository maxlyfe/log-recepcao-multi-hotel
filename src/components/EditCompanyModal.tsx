...
import type { Company, CreateInfoData, CreateContactData, UpdateCompanyData } from '../types/company';
...
export default function EditCompanyModal({ company, onClose, onSuccess }: EditCompanyModalProps) {
  ...
  const [infoFields, setInfoFields] = useState<CreateInfoData[]>([]);
  const [contactFields, setContactFields] = useState<CreateContactData[]>([]);

  useEffect(() => {
    if (company) {
      setName(company.name);
      // Garante que o array não seja nulo antes de mapear
      setInfoFields(company.company_info?.map(i => ({ info_label: i.info_label, info_value: i.info_value })) || []);
      setContactFields(company.company_contacts?.map(c => ({ contact_name: c.contact_name, contact_method: c.contact_method, contact_value: c.contact_value })) || []);
    }
  }, [company]);
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg max-w-2xl w-full max-h-[90vh] flex flex-col">
        <div className="sticky top-0 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 p-6">
          <div className="flex justify-between items-center">
            <h3 className="text-xl font-medium flex items-center gap-2"><Briefcase/> Editar Empresa</h3>
          ...
          <div className="space-y-4">
            ...
            {infoFields.map((field, index) => (
              <div key={index} className="flex items-center space-x-3 p-3 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
                ...
                <button onClick={() => handleRemoveField('info', index)} className="text-red-500 hover:text-red-700"><Trash2 className="h-4 w-4" /></button>
              </div>
            ))}
          ...
          <div className="space-y-4">
            ...
            {contactFields.map((field, index) => (
              <div key={index} className="grid grid-cols-12 gap-3 p-3 bg-gray-50 dark:bg-gray-700/50 rounded-lg items-center">
                ...
                <button onClick={() => handleRemoveField('contact', index)} className="col-span-1 text-red-500 hover:text-red-700"><Trash2 className="h-4 w-4" /></button>
              </div>
            ))}
