...
import { useTheme } from '../hooks/useTheme'; // 1. Importar o hook do tema
...
function NewLog() {
  const { isDark } = useTheme(); // 2. Chamar o hook para saber o estado do tema
  ...
  const monetaryKeys: (keyof ShiftValues)[] = ['cash_brl', 'envelope_brl', 'cash_usd'];
  ...
  // 3. Função atualizada para retornar cores para ambos os modos
  ...
  const renderShiftValuesComparison = (
      ...
      const renderItemCard = (key: keyof ShiftValues) => {
          ...
          const currencyType = key === 'cash_usd' ? 'USD' : 'BRL';
  ...
  const handleEditEntry = (entryId: string) => { const entry = currentLog?.entries.find(e => e.id === entryId); if (entry) { setEditingEntry(entryId); setEditText(entry.text); } };
  const handleSaveEdit = async (logId: string, entryId: string) => { await editLogEntry(logId, entryId, editText, currentLog?.receptionist || 'Unknown'); setEditingEntry(null); setEditText(''); };
  ...
  const handleShowHistory = async (type: 'entry' | 'values', id: string) => { await fetchEditHistory(type, id); setShowHistoryModal({ type, id }); };
  ...
  const renderMoneyDisplay = (values: ShiftValues) => ( <div className="money-display"><div className="space-y-2"><div className="flex items-center justify-between"><span className="text-sm">Fundo de Caixa:</span><span className="font-medium">{formatCurrency(values.cash_brl, 'BRL')}</span></div><div className="flex items-center justify-between"><span className="text-sm">Caixa do Dia:</span><span className="font-medium">{formatCurrency(values.envelope_brl, 'BRL')}</span></div><div className="flex items-center justify-between"><span className="text-sm">Dólar:</span><span className="font-medium">{formatCurrency(values.cash_usd, 'USD')}</span></div></div></div> );
  ...
  if (!currentLog) { return ( <div className="max-w-4xl mx-auto animate-fade-in"><div className="glass-effect rounded-2xl p-8 shadow-xl"><h2 className="text-2xl font-serif mb-6 flex items-center"><UserCircle className="h-8 w-8 text-blue-600 mr-3"/>Iniciar Novo Registro</h2><div className="space-y-6"><div><label className="block text-sm font-medium mb-2">Nome do Recepcionista</label><input type="text" value={receptionist} onChange={(e) => setReceptionist(e.target.value)} className="luxury-input w-full px-4 py-3 text-lg focus:outline-none" placeholder="Digite seu nome" disabled={isStartingLog}/></div> {previousLog && ( <div className="glass-effect p-4 rounded-lg"><div className="flex justify-between items-center mb-4"><h3 className="text-lg font-medium">Valores do Turno Anterior</h3><button onClick={copyPreviousValues} disabled={isStartingLog} className="flex items-center px-3 py-2 text-sm bg-blue-100 dark:bg-blue-900 text-blue-700 dark:text-blue-300 rounded-lg hover:bg-blue-200 dark:hover:bg-blue-800 transition-colors disabled:opacity-50"><Copy className="h-4 w-4 mr-2"/>Copiar Valores</button></div><div className="space-y-2"><p className="text-sm">Recepcionista: <span className="font-medium">{previousLog.receptionist}</span></p><p className="text-sm">Horário de Fechamento: <span className="font-medium">{previousLog.end_time ? new Date(previousLog.end_time).toLocaleTimeString() : 'Em andamento'}</span></p> {previousLog.endValues && ( <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4 p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg"><div><h4 className="text-sm font-medium mb-2">Valores Monetários:</h4><div className="space-y-1 text-sm"><div>Fundo de Caixa: <span className="font-medium">{formatCurrency(previousLog.endValues.cash_brl, 'BRL')}</span></div><div>Caixa do Dia: <span className="font-medium">{formatCurrency(previousLog.endValues.envelope_brl, 'BRL')}</span></div><div>Dólar: <span className="font-medium">{formatCurrency(previousLog.endValues.cash_usd, 'USD')}</span></div></div></div><div><h4 className="text-sm font-medium mb-2">Itens:</h4><div className="grid grid-cols-2 gap-2 text-sm">{Object.entries(previousLog.endValues).filter(([key]) => !monetaryKeys.includes(key as keyof ShiftValues)).map(([key, value]) => ( <div key={key}>{fieldLabels[key as keyof ShiftValues]}: <span className="font-medium">{String(value)}</span></div>))}</div></div></div>)}</div></div>)} <div className="space-y-4"><h3 className="text-lg font-medium">Valores Iniciais do Turno</h3>{renderShiftValues(startValues, (field, value) => setStartValues(prev => ({ ...prev, [field]: value })))}</div><button onClick={handleStartLog} disabled={!receptionist || isStartingLog} className="luxury-button w-full flex items-center justify-center px-6 py-4 rounded-xl text-white bg-gradient-to-r from-blue-600 to-blue-500 shadow-lg disabled:opacity-50 disabled:cursor-not-allowed">{isStartingLog ? ( <><Loader2 className="mr-2 h-5 w-5 animate-spin"/>Iniciando Turno...</>) : ( <><PlusCircle className="mr-2 h-5 w-5"/>Iniciar Turno</>)}</button></div></div></div> ); }
  return ( <div className="max-w-4xl mx-auto animate-fade-in"><div className="glass-effect rounded-2xl p-8 shadow-xl"><div className="flex justify-between items-center mb-8"><h2 className="text-2xl font-serif flex items-center"><Clock className="h-8 w-8 text-blue-600 mr-3"/>Registro em Andamento - {currentLog.receptionist}</h2><div className="flex items-center space-x-4">{renderMoneyDisplay(currentLog.startValues || defaultShiftValues)}<button onClick={() => setShowEndModal(true)} className="luxury-button px-6 py-3 rounded-xl text-white bg-gradient-to-r from-green-600 to-green-500 shadow-lg flex items-center"><Save className="h-5 w-5 mr-2"/>Finalizar Turno</button></div></div><div className="space-y-6"><div><label className="block text-sm font-medium mb-2">Nova Ocorrência</label><textarea value={entry} onChange={(e) => setEntry(e.target.value)} rows={3} className="luxury-input w-full px-4 py-3 text-lg focus:outline-none rounded-lg" placeholder={replyTo ? "Adicionar resposta..." : "Descreva a ocorrência..."}/></div><button onClick={handleAddEntry} disabled={!entry} className="luxury-button w-full flex items-center justify-center px-6 py-4 rounded-xl text-white bg-gradient-to-r from-blue-600 to-blue-500 shadow-lg disabled:opacity-50 disabled:cursor-not-allowed"><PlusCircle className="mr-2 h-5 w-5"/>{replyTo ? "Adicionar Resposta" : "Adicionar Ocorrência"}</button></div><div className="mt-8"><div className="flex justify-between items-center mb-6"><h3 className="text-xl font-serif">Ocorrências do Turno</h3><div className="flex items-center space-x-4">{currentLog.values_last_edited_at && ( <button onClick={() => handleShowHistory('values', currentLog.id)} className="flex items-center text-sm text-blue-600 hover:text-blue-800"><History className="h-4 w-4 mr-1"/>Ver histórico de valores</button>)}<button onClick={handleEditValues} className="flex items-center px-3 py-2 text-sm bg-blue-100 text-blue-700 rounded-lg hover:bg-blue-200"><Edit3 className="h-4 w-4 mr-2"/>Editar Valores</button></div></div><div className="space-y-4">{renderEntries()}</div></div> {showEndModal && ( <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50"><div className="bg-white dark:bg-gray-800 rounded-2xl p-8 max-w-5xl w-full mx-4 max-h-[90vh] overflow-y-auto shadow-2xl"><h3 className="text-2xl font-serif mb-6">Confirmar Finalização do Turno</h3> {renderShiftValuesComparison( currentLog.startValues || defaultShiftValues, endValues, (field, value) => setEndValues(prev => ({ ...prev, [field]: value })))} <div className="mt-8 flex justify-end space-x-4"><button onClick={() => setShowEndModal(false)} className="px-6 py-2 text-gray-700 dark:text-gray-300 bg-gray-200 dark:bg-gray-700 rounded-lg hover:bg-gray-300 dark:hover:bg-gray-600">Cancelar</button><button onClick={handleFinishLog} className="luxury-button px-8 py-2 rounded-lg text-white bg-gradient-to-r from-green-600 to-green-500 shadow-md">Confirmar e Finalizar</button></div></div></div>)} {showHistoryModal && ( <EditHistoryModal history={editHistory} onClose={() => setShowHistoryModal(null)} type={showHistoryModal.type}/>)} {editingValues && ( <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50"><div className="bg-white dark:bg-gray-800 rounded-lg p-8 max-w-4xl w-full mx-4 max-h-[90vh] overflow-y-auto"><h3 className="text-xl font-medium mb-6">Editar Valores Iniciais</h3> {renderShiftValues(startValues, (field, value) => setStartValues(prev => ({ ...prev, [field]: value })))} <div className="mt-6 flex justify-end space-x-4"><button onClick={() => setEditingValues(false)} className="px-4 py-2 text-gray-600 dark:text-gray-400 hover:text-gray-800 dark:hover:text-gray-200">Cancelar</button><button onClick={handleSaveValues} className="luxury-button px-6 py-2 rounded-lg text-white bg-gradient-to-r from-green-600 to-green-500 shadow-md">Salvar Alterações</button></div></div></div>)} {showEndConfirmModal && ( <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50"><div className="bg-white dark:bg-gray-800 rounded-lg p-8 max-w-md w-full mx-4"><div className="flex items-center text-yellow-500 mb-4"><AlertTriangle className="h-6 w-6 mr-2"/><h3 className="text-xl font-medium">Atenção</h3></div><p className="text-gray-600 dark:text-gray-400 mb-6">Existem ocorrências em aberto ou em andamento. Tem certeza que deseja finalizar o turno?</p><div className="flex justify-end space-x-4"><button onClick={() => setShowEndConfirmModal(false)} className="px-4 py-2 text-gray-600 dark:text-gray-400 hover:text-gray-800 dark:hover:text-gray-200">Cancelar</button><button onClick={confirmFinishWithOpenEntries} className="px-6 py-2 bg-yellow-500 text-white rounded-lg hover:bg-yellow-600">Finalizar Mesmo Assim</button></div></div></div>)}</div></div> );
}
